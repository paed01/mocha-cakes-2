...
[![Build](https://github.com/iensu/mocha-cakes-2/actions/workflows/build.yaml/badge.svg)](https://github.com/iensu/mocha-cakes-2/actions/workflows/build.yaml)
...
Or set it in your [`.mocharc`](https://mochajs.org/#configuring-mocha-nodejs) config file:
...
Mocha itself ships no types, so you also need `@types/mocha`:

```
npm install --save-dev typescript @types/mocha
```
...
To run `.ts` test files, either rely on Node's built-in type stripping (Node 22.18+ / 24+):

```
mocha --ui mocha-cakes-2 --extension ts ...
```

or use [tsx](https://tsx.is/) on older Node versions or when your tests need TypeScript features that Node cannot strip (enums, decorators, ...):

```
npm install --save-dev tsx
mocha --require tsx/cjs --ui mocha-cakes-2 --extension ts ...
```
